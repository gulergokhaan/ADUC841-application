...
float integral = 0, turev = 0, öncekiintegraldegeri = 0;  
...
void PID_Control(void) {
    // ADC'den deger okuma  
    istenen_deger = Read_ADC(1); // ADC'den deger oku  
    olculen_deger = Read_ADC(0); // ADC'den deger oku  
    ...
    // Integral terimi  
    integral = öncekiintegraldegeri + hata;  // Integral degerini biriktir  
    ui = (float)(Ki * integral);
    öncekiintegraldegeri = integral;
